...
async function getCardsWithDueDates() {
  ...
  for (const boardId of BOARD_IDS) {
    console.log(`📋 Fetching cards from board ${boardId}`);
...
function formatSummary(userItemsWithDates) {
  ...
  if (categorized.overdue.length) {
    summary.push(`⚠️<b>Overdue</b>`, ...indentList(categorized.overdue));
  }

  if (categorized.today.length) {
    summary.push(`\n<b>Today (${formatMonthDay(now)})</b>`, ...indentList(categorized.today));
  }
  ...
  if (categorized.future.length) {
    summary.push(`\n<b>Future</b>`, ...indentList(categorized.future));
  }
